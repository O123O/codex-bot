...
Slack support is not implemented in this release. The backend already separates normalized chat messages, attachments, delivery, and assistant routing from Telegram-specific transport, so a future Slack adapter can use the same contracts without changing Codex session identity.
