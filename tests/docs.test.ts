...
test("SSH worker guides document supported endpoints and the source-checkout fixture", async () => {
  ...
  for (const required of ["SSH worker endpoints", "endpoints.json", "0.144.4 or newer", "tmux -L qiyan-bot", "list-sessions", "disconnect_endpoint", "restart_endpoint", "unavailable SSH endpoint", "capacity stays reserved"]) {
