...
6. Resolve and expose the exact protocol PID from `LocalEndpoint` after successful initialization, bind it to that child generation, and clear it on stop, exit, startup failure, and unavailability. Extend the existing delayed-old-child restart test to prove an old generation cannot clear or replace the new generation's PID. Change production and integration wiring to use that PID, and require the request socket owner to equal it exactly.
...
9. Exercise the existing worker endpoint fixture by starting a worker thread without manager config and exhaustively paging `mcpServerStatus/list` for that thread; assert the reserved `codex_bot_manager` name is absent.
