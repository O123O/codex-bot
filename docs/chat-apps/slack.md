...
Search results are transient and newest-first. `search_slack.channel_id` defaults to `null` (all accessible channels); set it to a Slack channel ID to pass Slack's structured channel scope. QiYan retains no durable search-result report or SQLite receipt, returns at most 30 matches and 3,000 rendered words, and tells the assistant when pagination is incomplete or output was truncated. A result over 1,000 words or 16 KiB is written to an owner-only service-lifetime temporary JSON file; the tool returns its path and warns QiYan to read or query that file instead of repeating the search. Ask for a narrower query or date range when warned. `get_slack_mentions` means exact owner-mention matches returned by Slack search; Slack exposes no API that reproduces the Activity feed.
...
QiYan currently maintains one Socket Mode connection and replaces it with a fresh Slack URL when the connection becomes unhealthy. Slack supports multiple concurrent Socket Mode connections and may route each payload to any one of them. If repeated metadata-only measurements show frequent event-to-admission delays caused by reconnect gaps, overlapping connections are a potential future reliability improvement. They are not enabled by default because they require additional connection-lifecycle coordination while preserving shared event deduplication. See Slack's [Socket Mode connection guidance](https://docs.slack.dev/apis/events-api/using-socket-mode/#using-multiple-connections).
