...
const notification = (method: AssistantLifecycleNotification["method"], id: string): AssistantLifecycleNotification => ({
  method,
...
test("startup lifecycle notifications drain in arrival order including arrivals during activation", async () => {
  ...
  const handle = async (entry: AssistantLifecycleNotification) => {
