...
const packageName = "qiyan-bot" as const;
...
export async function readPackageInfo(moduleUrl: string = import.meta.url): Promise<PackageInfo> {
  ...
  while (true) {
    ...
    try {
      ...
      if (!isMissing(error)) throw new AppError("CONFIGURATION_ERROR", "could not read qiyan-bot package metadata");
    }
    if (source !== undefined) {
      ...
      try {
        ...
        throw new AppError("CONFIGURATION_ERROR", "invalid qiyan-bot package metadata");
      }
      if (!isRecord(manifest) || manifest.name !== packageName) {
        throw new AppError("CONFIGURATION_ERROR", "executable is not a qiyan-bot package");
      }
  ...
  throw new AppError("CONFIGURATION_ERROR", "could not locate qiyan-bot package metadata");
}
