...
The archive contains the bundled command and assistant templates. It does not install TypeScript source, build tools, or production npm dependencies.

The first versioned GitHub Release must exist before the stable `releases/latest/download/qiyan-bot.tgz` URL works. Until then, use the source-archive path below.
...
Removing the package does not remove bot state or assistant authentication:

```bash
npm uninstall --global --prefix "$HOME/.local" qiyan-bot
```

State remains in the separately configured data, registry, and assistant directories until you explicitly remove it.
