...
export const ASSISTANT_TOOL_SCHEMAS = {
  ...
  collect_messages: z.object({ nickname, count: z.number().int().positive().describe("how many of the most-recent final messages to deliver") }).strict(),
  ...
  prepare_chat_attachment: z.object({ owner: z.string().min(1).describe("nickname of the session that owns the file"), relative_path: z.string().min(1).describe("file path relative to the owner's project directory") }).strict(),
...
export const TOOL_DESCRIPTIONS: Partial<Record<AssistantToolName, string>> = {
  list_managed_sessions: "List all sessions QiYan manages (nickname, endpoint, provider codex|claude, thread, live status).",
  ...
  rename_session: "Rename a managed session's nickname (QiYan-side only; native thread unchanged).",
  unadopt_session: "Release a session WITHOUT archiving its native thread — it stays discoverable/re-adoptable.",
  archive_session: "Release a session AND archive its native thread so it drops out of default discovery.",
  ...
  set_goal: "Set/replace a worker's goal; QiYan auto-drives it after each turn until the worker ends it via set_goal_status — or (Claude) up to 50 turns then it pauses budgetLimited (resume_goal continues). Never mark a goal complete yourself.",
  pause_goal: "Pause the goal so QiYan stops auto-driving it (resume_goal continues).",
